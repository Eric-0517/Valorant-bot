...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (trackerOverview?.data?.data && Array.isArray(trackerOverview.data.data)) {
    ...
    // --- 情況 B: HenrikDev API 格式 (根據近期歷史賽事統計特務) ---
    else if (trackerOverview?.data?.data && Array.isArray(trackerOverview.data.data)) {
      const agentMap = {};

      trackerOverview.data.data.forEach((match) => {
