...
    
    // 找出發起查詢的玩家資訊
    const decodedPlayerID = decodeURIComponent(playerID).toLowerCase();
    const targetPlayer = players.find((p) => {
      const fullName = `${p.name}#${p.tag}`.toLowerCase();
      return fullName === decodedPlayerID || decodedPlayerID.includes(p.name?.toLowerCase());
    }) || players[0];
